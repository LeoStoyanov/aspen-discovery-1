	...
	function editObject($objectAction, $structure) {
		...
		if (empty($id) && $errorOccurred) {
			...
			if (is_null($redirectLocation)) {
				// Try to use our context-aware return URL first
...
;	}
	...
	private function storeListContext(): void {
		...
		// Check if this is coming from a list page by looking at the referer
		if (isset($_SERVER['HTTP_REFERER'])) {
			...
			if (isset($parsedUrl['query'])) {
				...
				if (isset($queryParams['objectAction']) && $queryParams['objectAction'] === 'list') {
					...
					// Store any special parameters like formId or pollId
					if (isset($queryParams['formId'])) {
						$listContext['formId'] = $queryParams['formId'];
					}
					if (isset($queryParams['pollId'])) {
						$listContext['pollId'] = $queryParams['pollId'];
					}
				}
			}
		}
		...
		// Store in session with a key specific to this module/tool
	...
	public function getReturnToListUrl($object = null): string {
		...
		// Add preserved context parameters
		$queryParams = [];
		...
		// Add special object-specific parameters if present
		if (!empty($object)) {
			if (!empty($object->formId)) {
				$queryParams['formId'] = $object->formId;
			}
			if (!empty($object->pollId)) {
				$queryParams['pollId'] = $object->pollId;
			}
		}
		
		// Add scroll positioning if we have an ID to scroll to
